...
def twython_check_auth(tw, logger=astra.baselogger):
    ...
    try:
        ...
        logger.info("Twython is authenticated as "
                    + colored.stylize(creds['name'], colored.fg("red")))
...
def twython_search(tw, searchstring, logger=astra.baselogger, tcount=100, maxtries=10):
    ...
    while len(results) < tcount:
        ...
        maxid = results[-1]['id']
        results = results + \
            tw.search(q=searchstring, count=tcount, max_id=maxid)['statuses']
        karma.wait(logger=logger)
        tries += 1
    logger.info("Got " + str(len(results))
                + " for search query " + searchstring)
...
def twython_get_ntweets_for_search(tw, search, tcount, geocode=None, maxtries=0, logger=astra.baselogger):
    ...
    p = pandas.DataFrame(tw.search(q=search, count=tcount,
                                   tweet_mode="extended")['statuses'])
    if len(p) > 0:
        tweetdf = p.drop_duplicates(
            subset="full_text", keep="first").reset_index()
    else:
    ...
    while len(tweetdf) < tcount:
        ...
        if "id" in p.columns:
            p = pandas.DataFrame(tw.search(
                q=search, count=tcount, tweet_mode="extended", max_id=p['id'].iloc[-1])['statuses'])
        else:
        ...
        tweetdf = tweetdf.drop_duplicates(
            subset="full_text", keep="first").reset_index(drop=True)
...
def get_tweet_density(tw, screen_name, logger=astra.baselogger):
    ...
    if "created_at" in last100.columns:
        ...
        logger.info(
            "Calculating density as number of tweets divided or 100 by hours since oldest tweet or 100th oldest tweet")
...
def get_mention_density(tw, screen_name, logger=astra.baselogger):
    ...
    if "created_at" in last100.columns:
        ...
        if len(last100) < 100:
            ...
            logger.info("Account " + screen_name + " has more than 100 mentions")
        logger.info(
            "Calculating density as number of mentions divided by  hours since oldest mention or 100th oldest mention")
        m = get_age(last100.createdts.min())
        density = len(last100) / m
        return round(density, 3)
    else:
