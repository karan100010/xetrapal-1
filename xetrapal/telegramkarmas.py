...
def poll(botname, botstop):
    while True:
        if not os.path.exists(botstop):
            botname.logger.info(
                u"{} waiting for messages. Touch {} to stop".format(botname.name, botstop))
            ...
            try:
                ...
                botname.logger.info(
                    u"{} got {} messages".format(botname.name, len(p)))
                if len(p) > 0:
                    for update in p:
            ...
            botname.logger.info(
                u"Stopfile found, {} exiting bot update loop".format(botname.name))
            break
...
def messageparser(bot, update, handler=None, logger=astra.baselogger):
    if update.message:
        ...
        logger.info(u"ID: {}\nSource Message: {}\nResponse: {}\nLocation: {}".format(
            reply_id, source_message, response, location))
        ...
        logger.info(u"ID: {}\nSource Message: {}\nResponse: {}\nLocation: {}".format(
            source_message, response, location))
    else:
        logger.info(
            u"This makes no sense, a message with no callback and no message")
    parsed_update = {"source_message": source_message,
                     "response": response, "location": location, "reply_id": reply_id}
